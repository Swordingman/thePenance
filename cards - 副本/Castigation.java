...
public class Castigation extends CustomCard {

    // 2. 卡牌ID：必须要和 Character 类里 getStartingDeck 写的一模一样
    // 建议格式：ModID:CardName
    public static final String ID = PenanceMod.makeID("Castigation");
    ...
    private static final int UPGRADE_PLUS_DMG = 2; // 升级增加的伤害
    private static final int BARRIER = 4; // 屏障值
    private static final int UPGRADE_PLUS_BARRIER = 2; // 升级增加的屏障倍率

    public Castigation() {
        super(ID, NAME, IMG, COST, DESCRIPTION, TYPE, COLOR, RARITY, TARGET);
        this.baseDamage = DAMAGE;
        this.baseMagicNumber = this.magicNumber =BARRIER;
    }
    ...
    public void upgrade() {
        if (!upgraded) {
            ...
            upgradeMagicNumber(UPGRADE_PLUS_BARRIER);
